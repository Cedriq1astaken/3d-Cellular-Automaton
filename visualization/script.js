...
for (let i = -grid_size; i < grid_size; i++) {
    for (let j = -grid_size; j < grid_size; j++) {
        for (let k = -grid_size; k < grid_size; k++) {
            ...
            const mat = new THREE.MeshBasicMaterial({ color: 0xff0011, flatShading: true });
...
function simulate() {
    ...
    cubes.forEach(({cube, i, j, k }) => {
        cube.visible = !!automaton.getState(i + grid_size, j + grid_size, k + grid_size);
    });
}
...
function animate() {
    ...
    wireframeCube.rotation.y += 0.001; // smooth rotation
    simulate();
